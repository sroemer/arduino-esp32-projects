...
static const uint8_t PIN_MOISTURE_SENSOR = A2; // ADC

// adc and moisture sensor definitions
static const uint8_t ADC_RESOLUTION    = 8;
static const uint8_t MOISTURE_SENSOR_MIN_VALUE = 160;
static const uint8_t MOISTURE_SENSOR_MAX_VALUE = 240;
...
static const char*    MQTT_PUB_TOPIC_MOISTURE      = "/Moisture";
...
static const uint32_t LOOP_COUNTER_RESET_VALUE  = 40;  // interval of loop actions = LOOP_COUNTER_RESET_VALUE*LOOP_INTERVAL;
...
void loop() {
  ...
  if(loop_counter == LOOP_COUNTER_ACTION_VALUE){
    ...
    uint16_t moisture_value = analogRead(PIN_MOISTURE_SENSOR);
    if(moisture_value>MOISTURE_SENSOR_MAX_VALUE)      moisture_value = MOISTURE_SENSOR_MAX_VALUE;
    else if(moisture_value<MOISTURE_SENSOR_MIN_VALUE) moisture_value = MOISTURE_SENSOR_MIN_VALUE;
    long moisture_percentage = map(moisture_value, MOISTURE_SENSOR_MAX_VALUE, MOISTURE_SENSOR_MIN_VALUE, 0, 100); // map value to percentage
    ...
    mqtt_publish(bmp280_temperature, bmp280_pressure, moisture_percentage);
  }
...
// =====================================================================================================================
void mqtt_publish(int8_t temperature, uint32_t pressure, uint8_t moisture){
  ...
  // moisture
  snprintf(szMqttTopic, sizeof(szMqttTopic), "%s%s", WiFi.getHostname(), MQTT_PUB_TOPIC_MOISTURE);
  char szMoisture[8];
  snprintf(szMoisture, sizeof(szMoisture), "%u", moisture);
  if(!mqttClient.publish(szMqttTopic, szMoisture)) {
